...
test("multiplying the inverse of a scale matrix", () => {
  const transform = new scale(2, 3, 4);
  const vector = new Vector(-4, 6, 8);
...
test("rotating a point around the x-axis", () => {
  ...
  const hq = rotateX(Math.PI / 4);
  const fq = rotateX(Math.PI / 2);
...
test("rotation around the y-axis", () => {
  ...
  const hq = rotateY(Math.PI / 4);
  const fq = rotateY(Math.PI / 2);
  expect(hq).toBeDefined();
  expectToBeCloseToTuple(
    hq.multiply(point),
...
test("rotation around the z-axis", () => {
  const point = new Point(0, 1, 0);
  const hq = rotateZ(Math.PI / 4);
  const fq = rotateZ(Math.PI / 2);
  expect(hq).toBeDefined();
  expectToBeCloseToTuple(
    hq.multiply(point),
...
test("individual transformations are applied in sequence", () => {
  ...
  const a = rotateX(Math.PI / 2);
  const b = scale(5, 5, 5);
  const c = translate(10, 5, 7);
...
test("the view transformation moves the world", () => {
  ...
  const up = new Vector(0, 1, 0);
  const t = view(from, to, up);
  expect(t).toEqual(translate(0, 0, -8));
});
