...
test("lighting with the eye between the light and the surface", () => {
  ...
  const obj = Sphere.of();
  expect(lighting(obj, light, p, eye, normal)).toEqual(
...
test("lighting with the eye between the light and surface, eye offset 45deg", () => {
  ...
  const obj = Sphere.of();
  expect(lighting(obj, light, p, eye, normal)).toEqual(Color.white);
});
...
test("lighting with eye opposite surface, light offset 45deg", () => {
  ...
  const obj = Sphere.of();
  const color = lighting(obj, light, p, eye, normal);
...
test("lighting with the surface in shadow", () => {
  ...
  const obj = Sphere.of();
  const inShadow = true;
  const color = lighting(obj, light, p, eye, normal, inShadow);
  expect(color).toEqual(Color.of({ r: 0.1, g: 0.1, b: 0.1 }));
});

test("lighting with a pattern applied", () => {
  const material = Material.of({
  ...
  const obj = Sphere.of({ material });
  const c1 = lighting(obj, light, Point.of({ x: 0.9 }), eye, normal, false);
  const c2 = lighting(obj, light, Point.of({ x: 1.1 }), eye, normal, false);
